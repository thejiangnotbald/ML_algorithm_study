...
for plot_index in range(numbers_to_display):
    # 读取数据
    digit = data[plot_index:plot_index + 1].values # 每次取一行
    digit_label = digit[0][0]
    digit_pixels = digit[0][1:]   # [784,]

    # 正方形的图像大小
    image_size = int(math.sqrt(digit_pixels.shape[0]))
    
    # 转换成图像形式
    frame = digit_pixels.reshape((image_size, image_size))  # [28,28]
    
    # 将当前的绘图区域分割成一个 num_cells x num_cells 的网格，并将下一个要绘制的图像放置在网格中的第 plot_index + 1 个位置。
...
# 训练参数
# 自己训练时最好把 logistic_regression.py文件中的 'if not result.success：'注释掉，不然会报错
max_iterations = 10000    # 这个要非常大，测试 100000 也没有训练成功
polynomial_degree = 0  
sinusoid_degree = 0  
normalize_data = True  
